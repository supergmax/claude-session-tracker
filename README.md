...
| 📈 **`token_dashboard.html`** | The same data as a **local dashboard**: open it in your browser, refresh after each response. Generated by the hooks themselves — **zero extra tokens**, no cloud, no artifact. Disable it with `{"dashboard": false}` in `.claude/session-tracker/config.json` if you only want Markdown. |
...
├── token_dashboard.html                # 📈 the local dashboard (optional, zero tokens)
├── story_log.md                        # 📖 your prompt log
└── .claude/session-tracker/
    ├── state.json                      # resume state (local, per project)
    ├── config.json                     # optional: {"dashboard": false}, per-project pricing
